...
import { deleteSkinTwinScenario, getSkinTwinScenario } from "@/lib/acnetrex/skin-twin/scenarios";
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!idSchema.safeParse(id).success) {
    return NextResponse.json({ ok: false, error: "invalid_scenario_id" }, { status: 400 });
  }
  try {
    const deleted = await deleteSkinTwinScenario(auth.userId, id);
    return deleted
