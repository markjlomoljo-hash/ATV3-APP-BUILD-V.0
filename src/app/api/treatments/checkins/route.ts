...
export async function GET(request: Request) {
  ...
  try {
    return NextResponse.json({ ok: true, checkins: await listTreatmentCheckins(auth.userId, parsedPlanId.data) });
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  const parsed = treatmentCheckinRequestSchema.safeParse(body.value);
  if (!parsed.success) return jsonError("invalid_treatment_checkin_payload", 400, parsed.error.issues);

  try {
    getDb();
    const result = await executeIdempotent({
      actorId: auth.userId,
      scope: "treatment-checkin",
      key: idempotencyKey.data,
      method: "POST",
      route: "/api/treatments/checkins",
      payload: parsed.data,
      execute: async () => {
        const checkin = await createTreatmentCheckin(auth.userId, parsed.data);
        return {
          status: 201,
          reference: { ok: true, checkin },
          resourceType: "treatment_checkin",
