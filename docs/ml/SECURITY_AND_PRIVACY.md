...
Known boundaries: the new Supabase migration is not live-executed; Cloud Run IAM/network posture is not live-verified; no secret scan can prove secrets absent from external systems; physical-device secure storage is unverified. Never paste credentials into issues, logs, documentation, or chat.
