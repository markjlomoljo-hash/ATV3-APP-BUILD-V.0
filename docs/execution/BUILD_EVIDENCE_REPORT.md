...
## 2026-07-13 Skin Twin scenario readiness and queue boundary

- Added authenticated, idempotent `POST/GET /api/skin-twin/scenarios` and
  owner-scoped `GET /api/skin-twin/scenarios/:id` routes.
- Scenario creation requires `personal_learning = true`, validates the PRD
  variable and timeline schema, and counts only the authenticated user's
  `face_scans`, `sleep_logs`, and `food_logs` records.
- Requests below the minimum source-data gate are persisted as
  `insufficient_data` with `simulation = null`. Requests meeting the gate are
  persisted as `queued_for_cloud` and create a durable `ml_analysis_jobs`
  record plus an outbox event. No projection is generated by this boundary.
- Added the Skin Twin scenario panel to `/skin-twin`, `/skin-twin/scenarios`,
  and `/skin-twin/history`. It exposes timeline and variable controls, loads
  owner-scoped history, and explicitly states that projections require a real
  validated worker result.
- Applied Supabase migration `20260713110859_skin_twin_scenario_queue_contract`.
  Live `information_schema` verification confirmed `scenario_payload`,
  `window`, `status`, `source_record_refs`, `model_version`, `confidence`,
  and `uncertainty` on `public.skin_twin_snapshots`.

### Skin Twin validation

| Command / check | Result |
|---|---|
| `npm.cmd test -- src/lib/acnetrex/skin-twin src/app/api/skin-twin --run` | Pass: 2 files, 8 tests |
| `npm.cmd test` | Pass: 33 files, 164 tests |
| `npm.cmd run typecheck` | Pass |
| `npm.cmd run build` | Pass: both Skin Twin API routes included |
| `npm.cmd run lint` | Pass |
| `npm.cmd run test:coverage` | Pass: 79.91% statements, 69.73% branches |
| `npm.cmd run test:e2e` | Pass: route smoke for 66 routes |
| `GET http://127.0.0.1:3200/skin-twin` | HTTP 200 on existing dev preview |
| `GET http://127.0.0.1:3200/api/skin-twin/scenarios` without bearer | HTTP 401 |
| `GET https://atv-3-app-build-v-0.vercel.app/skin-twin` | HTTP 200 on current production deployment |
| Supabase migration and column query | Applied as `20260713110859`; all seven columns verified |
| Playwright visual browser | Not run: browser executable is not installed in this environment |

This proves the readiness-gated persistence and queue boundary. It does not
