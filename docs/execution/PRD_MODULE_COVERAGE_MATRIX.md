...
| Treatment Plan Center | `/treatments`, `/treatments/checkins`, `/log/treatment` | treatment draft/check-in schemas | owner-scoped treatment plan/check-in services | live canonical treatment plans/check-ins; tasks remain separate | provider-directed safety gate | treatment route/service tests | partial |
| Task Board/Gamification | `/tasks`, `/gamification` | task workflow + canonical gamification schema | owner-scoped treatment task service and read-only gamification summary | `treatment_tasks`, `gamification`, `badges`, `user_badges` | real-data-only gates | task route/service + gamification reader tests | partial |
| Reports/Exports/Profile | `/reports`, `/reports/history`, `/reports/export`, `/export`, `/profile` | validated report request form and section selection | authenticated request/history/download workflow | report/export/profile tables; treatment compiler aligned to live schema | missing-data report and typed failure states | route and API tests | partial |
